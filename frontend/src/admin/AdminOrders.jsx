                    ...
                    <tbody>
                        {orders.map(order => (
                            <tr key={order._id} style={rowStyle}>
                                ...
                                <td style={tdStyle}>{order.userId?.name || `Deleted User - ${order.address.fullName} ` }</td>
                                ...
                                <td style={tdStyle}>
                                    <select
                                        value={editStatuses[order._id] || order.status}
                                        onChange={(e) => setEditStatuses({ ...editStatuses, [order._id]: e.target.value })}
                                    ...
                                    <td style={tdStyle}>
                                        <button className='btn' style={{marginLeft:'20px'}} onClick={() => updateStatus(order._id , editStatuses[order._id] || order.status)}> Update</button>
                                        </td>
                            </tr>
                        ))}
                    </tbody>
