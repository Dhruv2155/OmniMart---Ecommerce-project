...
const AddProduct = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!image) return alert('Please select an image for the product')
        ...
        try {
            ...
            const resData = await res.json()
             if(res.ok){
                ...
                alert(resData.message || 'Error creating product')
            }
        } catch (error) {
           console.error(error)   
        }
...
return (
    <div style={{ maxWidth: '600px', margin: '40px auto', background: '#18181b', padding: '40px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        />
        <input 
          type="number" min="0" placeholder="Stock Quantity" required 
